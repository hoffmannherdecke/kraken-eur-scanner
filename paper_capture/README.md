...
Kraken WebSocket transport, market metrics and REST context are implemented in
the shared `market_data/` package. This directory is the Paper-only
candidate-discovery, capture-journal and artifact adapter; do not add another
copy of market transport or metric calculation here. The future live
assessment is not wired and remains disabled by `market_data/runtime.py`.
